...
class OrderNotifier extends AsyncNotifier<List<OrderModel>> {
  ...
  Future<bool> createOrderFromCart({
    ...
    try {
      ...
      final chefId = cart.chefId ?? (cart.items.isNotEmpty ? cart.items.first.chefId : null);
      ...
      if (promo != null) {
        final discount = (cart.total * promo.discountPercentage).clamp(0, promo.maxDiscount);
        finalTotal = (cart.total + deliveryFee - discount).clamp(0, double.infinity);
      }
      ...
      final chefName = chefUser?.kitchenName.isNotEmpty == true ? chefUser!.kitchenName : (chefUser?.name ?? 'Home Chef');
      final chefAddress = chefUser?.address ?? 'Unknown Kitchen Address';
      ...
      for (var item in cart.items) {
        ...
        final dish = dishes.where((d) => d.id == item.dishId).isEmpty ? null : dishes.firstWhere((d) => d.id == item.dishId);
        if (dish != null) {
    ...
    try {
      ...
      final enriched = orders.map((order) {
        if (order.dishImagePath.isEmpty) {
          ...
          if (dish != null) return order.copyWith(dishImagePath: dish.imagePath);
        }
        return order;
      }).toList();
  ...
  Future<bool> unassignOrder(OrderModel order) async {
    try {
      ...
      await _orderService.updateOrderStatus(order.id, OrderStatus.ready, riderId: '');
  ...
  Future<bool> updateOrderStatus({
    ...
    try {
      ...
      await _orderService.updateOrderStatus(
        ...
        refundStatus: refundStatus,
        riderId: (currentUser.role == UserRole.rider && newStatus == OrderStatus.pickedUp) || (currentUser.role == UserRole.admin && riderId != null)
      ...
      if (updatedOrder != null && config != null) {
        ...
        if (newStatus == OrderStatus.canceled && updatedOrder.paymentMethod == PaymentMethod.wallet) {
           await ref.read(walletProvider(updatedOrder.customerId).notifier)
               .issueRefund(updatedOrder.totalPrice, updatedOrder.id);
        }

        // RULE 2: Chef Penalty if cancelling accepted order
        if (newStatus == OrderStatus.rejected && order.status != OrderStatus.pending) {
  ...
  Future<void> _updateCategoryPreference(String userId, String category) async {
    ...
    if (user != null) {
      ...
      await authNotifier.updateProfile(user.copyWith(orderedCategories: updatedCategories));
    }
  }
}
