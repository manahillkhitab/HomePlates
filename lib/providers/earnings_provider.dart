...
final chefEarningsProvider = Provider.family<List<OrderModel>, String>((
  ref,
  chefId,
) {
  ...
  return allOrders.value
          ?.where(
            (order) =>
                order.chefId == chefId && order.status == OrderStatus.delivered,
          )
          .toList() ??
      [];
});
...
final riderEarningsProvider = Provider.family<List<OrderModel>, String>((
  ref,
  riderId,
) {
  final allOrders = ref.watch(allOrdersStreamProvider);
...
// Stats Provider
final earningsStatsProvider = Provider.family<EarningsState, String>((
  ref,
  userId,
) {
