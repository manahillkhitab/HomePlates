...
final socialProvider = StateNotifierProvider<SocialNotifier, List<PostModel>>((
  ref,
) {
...
class SocialNotifier extends StateNotifier<List<PostModel>> {
  ...
  Future<void> loadPosts() async {
    try {
      ...
      final response = await Supabase.instance.client
          .from('stories')
          .select(
            '*, profiles:chef_id(name, kitchen_name, profile_image_url)',
      ...
      final posts = (response as List).map((data) {
        ...
        final chefName =
            profile?['kitchen_name'] as String? ?? profile?['name'] as String?;
        return PostModel(
          id: data['id'],
          chefId: data['chef_id'],
  ...
  Future<void> unlikePost(String postId) async {
    state = [
      for (final post in state)
        if (post.id == postId)
          post.copyWith(likes: (post.likes > 0 ? post.likes - 1 : 0))
        else
          post,
    ];
  }
