        ...
        child: CustomScrollView(
          ...
          slivers: [
            ...
            ValueListenableBuilder(
              ...
              ).listenable(keys: [widget.order.id]),
                    ...
                    delegate: SliverChildListDelegate([
                      ...
                      if (liveOrder.status == OrderStatus.pickedUp) ...[
                        ...
                        _buildContactActions(context, 'Rider'),
                          ...
                          liveOrder.status == OrderStatus.cooking ||
  ...
  Widget _buildTimeline(BuildContext context, OrderStatus currentStatus) {
    ...
    if (currentStatus == OrderStatus.rejected ||
      ...
      return Container(
        ...
        decoration: BoxDecoration(
          color: (isCanceled ? Colors.grey : Colors.redAccent).withValues(
            alpha: 0.05,
          ),
          ...
          afterLineStyle: LineStyle(
            color:
                (index < steps.length - 1 &&
                    currentStatus.index > stepStatus.index)
            ...
            child: Row(
              ...
              children: [
                ...
                Icon(
                  _getStatusIcon(stepStatus),
                  color: isActive
                      ? _getStatusColor(stepStatus).withValues(alpha: 0.6)
      ...
      if (context.mounted) {
        if (success) {
          ...
          ScaffoldMessenger.of(context).showSnackBar(
            const SnackBar(
              content: Text(
                'Failed to cancel order. It might already be in progress.',
              ),
            ),
          );
        }
  ...
  void _openChat(BuildContext context, String role) {
    ...
    if (targetId != null) {
      Navigator.push(
        ...
        MaterialPageRoute(
          builder: (context) =>
              ChatScreen(otherUserId: targetId!, otherUserName: targetName),
        ),
      );
    } else {
      ScaffoldMessenger.of(context).showSnackBar(
          ...
          ElevatedButton(
            onPressed: () {
              ...
              ScaffoldMessenger.of(context).showSnackBar(
                const SnackBar(
                  content: Text(
                    'Report submitted to Admin. We will review it shortly.',
                  ),
                ),
              );
            },
